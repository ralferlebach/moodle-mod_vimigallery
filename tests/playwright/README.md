# mod_vimigallery — Playwright user-story tests
...
These need a **running Moodle site** with mod_vimigallery installed. They do not run inside
...
```
cd tests/playwright
php seed.php > .env.sh          # creates the course, users and gallery fixture
set -a; . ./.env.sh; set +a
npm ci
...
The seed prints `VIMIGALLERY_*` environment variables that `support/env.ts` reads.
`VIMIGALLERY_BASE_URL` defaults to `http://localhost:8000`; override it if your
